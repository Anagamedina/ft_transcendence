<template>
	<MainLayout header-title="AquaGuard Frontend — Issue #4" sidebar-app-name="AquaGuard Test">
		...
		<div class="p-4 mb-4 bg-gray-100 rounded-lg text-sm">
			<p class="font-semibold text-gray-500 mb-1">[DEV] Estado de los stores</p>
			<p>Usuario: {{ authStore.user }}</p>
			<p>Sensores en el store: {{ sensorsStore.sensorCount }}</p>
			<p>Alertas: {{ alertsStore.alerts }}</p>
		</div>

		<div v-if="sensorsStore.sensors.length" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mt-4">
			<router-link
		...
		<p v-else class="text-gray-500 mt-4">No hay sensores disponibles.</p>

		<button
			@click="modalAbierto = true"
			class="mt-4 border-2 border-aqua-600 text-aqua-600 px-4 py-2 rounded-lg hover:bg-aqua-50 transition"
		>
			Abrir modal
		</button>

		<Modal :show="modalAbierto" title="Prueba de Modal" @close="modalAbierto = false">
