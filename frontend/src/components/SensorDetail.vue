<template>
  <div v-if="sensor" class="max-w-md">
    <div class="bg-white border-2 rounded-xl shadow-lg p-6" :class="status.borderClass">
      ...
      <div class="mt-6">
        <p v-if="hasReading" class="text-4xl font-bold text-aqua-900">
          {{ sensor.value }} <span class="text-lg font-medium text-gray-500">{{ sensor.unit }}</span>
        </p>
...
const hasReading = computed(() =>
  props.sensor?.value !== null && props.sensor?.value !== undefined && props.sensor?.value !== ''
)
</script>
